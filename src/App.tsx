...
if (editingPokemonId) {
// EDITING: We merge the updated data but keep its current location (so it doesn't accidentally move from PC to roster when edited)
setRoster(roster.map(p => p.id === pokemonData.id ? { ...p, ...pokemonData } : p));
setEditingPokemonId(null);
} else {
// NEW CATCH: Count how many Pokémon are currently NOT in the PC
const activeRosterCount = roster.filter(p => p.location !== 'pc').length;

  const newlyCaughtPokemon = {
    ...pokemonData,
    // If they already have 6 on their team, route this one to the PC
  ...
  // If this is their very first Pokémon ever, make it the active combatant
  if (roster.length === 0) {
...
<p className="text-slate-400 mb-4">5e Custom Campaign Manager</p>
  ...
  {!trainerData ? (
    <TrainerForm onSave={setTrainerData} />
  ) : (
    <div className="w-full max-w-4xl mt-8 flex flex-col gap-6">
      ...
      <div className="bg-slate-800 p-6 rounded-lg shadow-lg border border-slate-700">
        <div className="flex justify-between items-start mb-4 border-b border-slate-600 pb-4">
          ...
          <button
            onClick={() => setTrainerData(null)}
            className="text-sm bg-slate-700 hover:bg-slate-600 text-slate-300 py-1 px-3 rounded"
          >
            Edit Trainer
          </button>
        </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {Object.entries(trainerData.stats).map(([stat, value]) => (
                <div
                  key={stat}
                  className="bg-slate-700 p-2 rounded text-center flex-1 border border-slate-600"
                >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {trainerData.skills.map((skill: string) => (
                <span
                  key={skill}
                  className="bg-blue-900/50 text-blue-200 border border-blue-700/50 px-3 py-1 rounded-full text-sm"
                >
                  {skill}
                </span>
              ))}
            ...
            {activeRosterOnly.map((pokemon) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex flex-col gap-2 min-w-[130px]">
                      ...
                      <div className="bg-slate-800 px-3 py-1 rounded border border-slate-700 text-center">
                        <span className="text-slate-400 text-xs mr-2 font-semibold">
                          AC
                        </span>
                  ...
                  {/* BOTTOM ROW: Prepared Moves (NOW ONLY VISIBLE IF ACTIVE) */}
